...
class ForgotPasswordFormTest < PasswordResets
  ...
  test "test path with invalid email" do
...
class PasswordFormTest < PasswordResetForm
  ...
  test "reset with innactive user" do
  ...
  test "reset with right email but wrong token" do
    get edit_password_reset_path('boohahaha', email: @reset_user.email)
    assert_redirected_to root_url
  end
    ...
    patch(
      ...
      params: {
        ...
        user: {
          ...
          password_confirmation: "ehmbadpassword"
        }
      }
    )
