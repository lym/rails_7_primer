...
class Following < ActionDispatch::IntegrationTest
  def setup
    @user = users(:john_doe)
    @other = users(:jane_doe)
    log_in_as(@user)
  end
end

class FollowPagesTest < Following
  test "acccessing followed users' page" do
  ...
  test "accessing followers' page" do
...
class FollowTest < Following
  test "following a user without Hotwire" do
  ...
  test "following a user with Hotwire" do
    assert_difference "@user.following.count", 1 do
      post(
...
class UnfollowTest < Unfollow
  test "unfollow a user without Hotwire" do
    assert_difference "@user.following.count", -1 do
  ...
  test "unfollow a user with Hotwire" do
    assert_difference "@user.following.count", -1 do
